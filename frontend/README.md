...
**Tests:** `npm test` runs Karma/Jasmine (local browser). `npm run build` then `npm run test:e2e` runs Playwright smoke tests against the static build (`serve -s`). CI on `main` runs backend `dotnet test` and frontend build + Playwright only — see [docs/testing.md](../docs/testing.md).
